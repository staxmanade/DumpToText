...
$version = Get-Last-NuGet-Version 'DumpToText'
...
$projectOutputDestination = "$buildRoot\content\DumpToText"
rm $buildRoot -force -recurse -ErrorAction SilentlyContinue
mkdir $projectOutputDestination | out-null
cp .\DumpToText\DumpToText.cs $projectOutputDestination
$nuspecFile = "DumpToText.$newVersion.nuspec"
cp .\DumpToText.nuspec "$buildRoot\$nuspecFile"
pushd $buildRoot
